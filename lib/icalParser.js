...
// Loaded by service/eventProperties.js and service/eventQuery.js (service process)
...
/**
 * Resolve a TZID to a GLib.TimeZone. GLib.TimeZone.new() silently falls back
 * to UTC for identifiers it doesn't recognize (rather than throwing), so a
 * bad TZID otherwise produces a wrong-but-plausible timestamp instead of an
 * error. Some calendars (eg GNOME Calendar's local/offline calendars)
 * embed a VTIMEZONE with a globally-unique TZID of the form
 * "/<domain-containing-a-dot>/<Olson-zone-name>" instead of a plain IANA
 * id -- strip that prefix and retry with the trailing Olson name.
 * @param {string} tzid - TZID value from an iCal property
 * @returns {GLib.TimeZone}
 */
export function resolveTimezone(tzid) {
    const tryZone = id => {
        const tz = GLib.TimeZone.new(id);
        return tz.get_identifier() === id ? tz : null;
    };
    ...
    if (tzid.startsWith('/')) {
        const parts = tzid.slice(1).split('/');
        if (parts.length > 1 && parts[0].includes('.')) {
            tz = tryZone(parts.slice(1).join('/'));
            if (tz) return tz;
        }
    }

    return GLib.TimeZone.new(tzid);
}
...
export function extractIcalDateString(icalStr, propName) {
    const extracted = extractIcalProperty(icalStr, propName);
    if (!extracted || extracted.value.length < 8) return null;

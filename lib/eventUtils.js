...
// Loaded by ui/indicator.js (shell) and service.js + service/eventProperties.js (service)
...
export function hasRecurrenceId(event) {
    ...
    // libical stringifies a "null time" (no RECURRENCE-ID) as all zeros
    return recurid != null && recurid !== '' && recurid !== '00000000T000000';
}
...
/**
 * Check if event is all-day using heuristic
...
export function isAllDayEventHeuristic(startTime, endTime) {
    ...
    return isMidnight && duration >= dayMs && duration % dayMs === 0;
}

/**
 * Filter events that can appear in the panel countdown/current indicator
 * @param {object[]} events - Array of event objects
 ...
 */
function filterPanelEvents(events, options) {
    const {
        isAllDayEvent,
        isDeclinedEvent,
        isTentativeEvent,
    ...
    return events.filter(evt => {
        ...
        // Declined events can still appear in the menu based on settings.
...
/**
 * Get current events from an already-filtered list
 * @param {object[]} events - Filtered events
 * @param {function} getEventStart - Get event start time
 * @param {function} getEventEnd - Get event end time
 * @param {number} now - Current timestamp
 * @param {number} minRemainingMs - Minimum time remaining
 * @returns {object[]} Current meetings sorted by end time
 */
function getCurrentMeetingsFromFilteredEvents(events, getEventStart, getEventEnd, now, minRemainingMs) {
    const remainingThreshold = now + Math.max(0, minRemainingMs);
    return events.filter(evt => {
        const start = getEventStart(evt);
        const end = getEventEnd(evt);
        return start <= now && end > remainingThreshold;
    }).sort((a, b) => getEventEnd(a) - getEventEnd(b));
}

/**
 * Get currently ongoing meetings, sorted by end time (soonest first)
 * @param {object[]} events - Array of event objects
 ...
 * @param {number} [options.minRemainingMs=FIVE_MINUTES_MS] - Minimum time remaining
 * @returns {object[]} Current meetings sorted by end time
 */
export function getCurrentMeetings(events, options) {
    const {
        getEventStart,
        getEventEnd,
        isAllDayEvent,
        ...
        eventTypes = ['regular'],
        now = Date.now(),
        minRemainingMs = FIVE_MINUTES_MS,
    } = options;
    ...
    const relevantEvents = filterPanelEvents(events, {
        isAllDayEvent,
        isDeclinedEvent,
        isTentativeEvent,
        eventTypes,
    ...
    return getCurrentMeetingsFromFilteredEvents(
        relevantEvents,
        getEventStart,
        getEventEnd,
        now,
...
export function getNextMeeting(events, options) {
    ...
    if (showCurrentMeeting) {
        const currentEvents = getCurrentMeetingsFromFilteredEvents(
            relevantEvents,
            getEventStart,
            getEventEnd,
            now,
    ...
    sortEventsByStartTime(upcomingEvents, getEventStart);
